...
Write-Host "CASA School - Staging Runtime + Functional Safe Smoke Verification V1" -ForegroundColor Green
...
try {
    ...
    Assert-Hash "src\server\card-production\renewal-production.ts" $ExpectedRenewalHash
    ...
    $Evidence =
        Join-Path `
            $ProjectRoot `
            ".casa-backups\staging-runtime-functional-smoke-v1-$Stamp"
...
'@
    ...
    $Result =
        [ordered]@{
            createdAt =
                (Get-Date -Format o)
            proof =
                "CASA_STAGING_RUNTIME_FUNCTIONAL_SAFE_SMOKE_V1"
            classification =
                "GREEN"
            sourceAuthority =
                "main/a7aa668"
    ...
    Write-Host ""
    Write-Host "CASA STAGING RUNTIME + FUNCTIONAL SAFE SMOKE V1 IS GREEN" -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "CASA STAGING RUNTIME + FUNCTIONAL SAFE SMOKE V1 STOPPED: $($_.Exception.Message)" -ForegroundColor Red
