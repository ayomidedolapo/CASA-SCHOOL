...
Write-Host "CASA School - AWS Environment Isolation + Vercel OIDC Source Foundation V1R1" -ForegroundColor Green
...
'@
  ...
  $result = [ordered]@{
    ...
    proof = "CASA_AWS_ENVIRONMENT_ISOLATION_VERCEL_OIDC_FOUNDATION_V1R1"
    branch = $branch
    head = $head
    migrations = 30
  ...
  Write-Host ""
  Write-Host "CASA AWS ENVIRONMENT ISOLATION + VERCEL OIDC SOURCE FOUNDATION V1R1 IS GREEN" -ForegroundColor Green
  ...
  Restore-Source
  Write-Host ""
  Write-Host "CASA AWS ENVIRONMENT ISOLATION + VERCEL OIDC SOURCE FOUNDATION V1R1 STOPPED: $($_.Exception.Message)" -ForegroundColor Red
