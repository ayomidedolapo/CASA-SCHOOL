...
Write-Host "CASA School - Guarded Staging Full-Prefix Promotion 16 to 30 V1R2" -ForegroundColor Green
Write-Host "Promotes the exact verified Staging prefix from migration 16 to Development migration 30."
Write-Host "V1R2 keeps the native-array fix and replaces the coarse DML scanner with an exact migration-aware backfill contract for migrations 17 and 23, while stripping dollar-quoted function bodies to avoid false-positive UPDATE markers in migration 30."
...
try {
    ...
    $Evidence = Join-Path $ProjectRoot ".casa-backups\staging-full-prefix-16-to-30-v1r2-$Stamp"
    ...
    $Result = [ordered]@{
        ...
        proof = "CASA_STAGING_FULL_PREFIX_16_TO_30_V1R2"
    ...
    Write-Host ""
    Write-Host "CASA STAGING FULL-PREFIX PROMOTION 16 -> 30 V1R2 IS GREEN" -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "CASA STAGING FULL-PREFIX PROMOTION V1R2 STOPPED: $($_.Exception.Message)" -ForegroundColor Red
