...
Write-Host "CASA School - Guarded Staging Full-Prefix Promotion 16 to 30 V1R1" -ForegroundColor Green
Write-Host "Promotes the exact verified Staging prefix from migration 16 to Development migration 30."
Write-Host "V1R1 replaces the Windows PowerShell-incompatible generic List manifest accumulator with a native PowerShell array; promotion semantics are unchanged."
...
try {
    ...
    $Evidence = Join-Path $ProjectRoot ".casa-backups\staging-full-prefix-16-to-30-v1r1-$Stamp"
    ...
    $Result = [ordered]@{
        ...
        proof = "CASA_STAGING_FULL_PREFIX_16_TO_30_V1R1"
    ...
    Write-Host ""
    Write-Host "CASA STAGING FULL-PREFIX PROMOTION 16 -> 30 V1R1 IS GREEN" -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "CASA STAGING FULL-PREFIX PROMOTION V1R1 STOPPED: $($_.Exception.Message)" -ForegroundColor Red
