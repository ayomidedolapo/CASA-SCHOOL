...
try {
  Write-Host "CASA School - Permanent Card + Scanner Simplification V1R4" -ForegroundColor Green
  ...
  $Evidence = Join-Path $ProjectRoot ".casa-backups\permanent-card-scanner-simplification-v1r4-$stamp"
  ...
  $PatcherPath = Join-Path $Evidence "apply-permanent-card-scanner-v1r4.mjs"
  $patcher = @'
...
// 10) Synchronize the historical frontend regression's accepted Scanner hash if it is hash-pinned.
{
...
'@
  ...
  Write-Host ""
  Write-Host "CASA PERMANENT CARD + SCANNER SIMPLIFICATION V1R4 IS GREEN" -ForegroundColor Green
