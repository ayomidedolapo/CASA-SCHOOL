...
try {
  ...
  if (
    ...
    identityCardStatusCounts =
      Object.fromEntries(
        cardStatuses.rows.map(
          (row) => [
            String(
              row.status,
            ),
  ...
  const backfill = {
    schools:
      tableNames.includes(
        ...
        : null,
    schoolsMissingBranch:
      null,
    sectionsMissingBranchMapping:
      null,
    classArmsMissingBranchMapping:
      null,
    terminalsMissingBranchMapping:
      null,
    students:
      tableNames.includes(
        "students",
        ...
        : null,
    studentsMissingArrivalAssignment:
      null,
    arrivalAssignments:
      null,
    casaInternalMemberships:
      null,
  };
  ...
  if (
    ...
    backfill.sectionsMissingBranchMapping =
      Number(
        (
          await client.query(`
            select count(*)::int as count
  ...
  if (
    ...
    backfill.classArmsMissingBranchMapping =
      Number(
        (
          await client.query(`
            select count(*)::int as count
            ...
            where not exists (
              select 1
              from school_branch_class_arms m
              where m.school_id=a.school_id
                and m.class_arm_id=a.id
            )
          `)
        ).rows[0]?.count ??
        0,
  ...
  if (
    ...
    backfill.terminalsMissingBranchMapping =
      Number(
        (
          await client.query(`
            select count(*)::int as count
            ...
            where not exists (
              ...
              where m.school_id=t.school_id
                and m.terminal_id=t.id
            )
          `)
        ).rows[0]?.count ??
        0,
  ...
  if (
    ...
    backfill.studentsMissingArrivalAssignment =
      Number(
        (
          await client.query(`
            select count(*)::int as count
            ...
            where not exists (
              select 1
              from student_arrival_method_assignments a
  ...
  if (
    ...
    backfill.casaInternalMemberships =
      Number(
        (
          await client.query(`
            select count(*)::int as count
...
function Test-PendingMigrationSqlSafety(
    ...
    $ScannerPath =
        Join-Path `
            $Evidence `
            "pending-migration-sql-safety.mjs"
...
const MIGRATION17 =
  "20260830031700_school-real-world-foundation";

const MIGRATION23 =
  "20260902190300_wave1_transport_casa_internal";
...
function stripCommentsStringsAndDollarBodies(
  sql,
) {
  let out = "";
  let i = 0;
  ...
  while (
    ...
    if (
      ...
      if (
        ...
        throw new Error(
          "Quoted identifier parser did not advance.",
        );
      }
    ...
    if (
      ...
      if (
        ...
        if (
          ...
          throw new Error(
            `Unclosed dollar-quoted body: ${delimiter}`,
          );
        }
...
for (
  ...
  const normalized =
    stripCommentsStringsAndDollarBodies(
      raw,
    );
  ...
  const inserts =
    [
      ...normalized.matchAll(
        /\bINSERT\s+INTO\s+((?:"(?:[^"]|"")*"|[A-Za-z_][A-Za-z0-9_]*)(?:\s*\.\s*(?:"(?:[^"]|"")*"|[A-Za-z_][A-Za-z0-9_]*))?)/gi,
      ),
    ];
  ...
  if (
    ...
    if (
      ...
      findings.push({
        path:
          filePath,
        label:
          `UNAUTHORIZED INSERT target(s): ${observedTargets.join(", ")}`,
      });
    }
    else {
      const unknown =
      ...
      if (
        ...
        findings.push({
          path:
            filePath,
          label:
            `INSERT contract mismatch; observed=${observedTargets.join(",")} unknown=${unknown.join(",")} missing=${missingExpected.join(",")}`,
        });
      }
      else {
        approvedBackfills.push({
          migration:
            folder,
          insertTargets:
            observedTargets,
    ...
    findings.push({
      path:
        filePath,
      label:
        "Expected approved backfill INSERT statements are missing.",
    });
  }
}
...
console.log(
  JSON.stringify({
    checked:
      paths.length,
    topLevelUpdateDeleteTruncateCopyMerge:
      false,
    destructiveDropTableOrColumn:
      false,
    approvedBackfills,
    dollarQuotedBodiesIgnoredForDmlClassification:
      true,
  }),
);
'@
    ...
    if ($ExitCode -ne 0) {
        ...
        Stop-Phase "Pending migration SQL contract failed. No Staging write was attempted."
    }

    if ($Lines.Count -eq 0) {
        Stop-Phase "Pending migration SQL contract returned no result."
    }
    ...
    try {
        ...
        Stop-Phase "Pending migration SQL contract returned invalid JSON."
    }

    if (
        [int]$Contract.checked -ne 14 -or
        @($Contract.approvedBackfills).Count -ne 2 -or
        -not [bool]$Contract.dollarQuotedBodiesIgnoredForDmlClassification
    ) {
        Stop-Phase "Pending migration SQL contract returned an unexpected classification."
    }
    ...
    $Contract |
        ...
        Set-Content `
            -LiteralPath (
                Join-Path `
                    $Evidence `
                    "PENDING_MIGRATION_SQL_CONTRACT.json"
            ) `
            -Encoding UTF8

    Write-Host "  pending migrations checked:   14"
    Write-Host "  migration 17 backfill:         APPROVED / Main Campus + structure/terminal mappings"
    Write-Host "  migration 23 backfill:         APPROVED / missing students -> INDEPENDENT arrival assignment"
    Write-Host "  top-level UPDATE/DELETE/etc:   NONE"
    Write-Host "  migration 30 trigger body:     DOLLAR-QUOTED / EXCLUDED FROM TOP-LEVEL DML CLASSIFICATION"
    Write-Host "  PostgreSQL ON UPDATE clauses:   NOT DATA DML / STATEMENT-AWARE CLASSIFICATION"
...
Write-Host "CASA School - Guarded Staging Full-Prefix Promotion 16 to 30 V1R3" -ForegroundColor Green
Write-Host "Promotes the exact verified Staging prefix from migration 16 to Development migration 30."
Write-Host "V1R3 keeps the exact migration-aware backfill contracts and makes UPDATE detection statement-start aware, so PostgreSQL ON UPDATE clauses and trigger declarations cannot be mistaken for row-rewrite DML."
...
try {
    ...
    $Evidence = Join-Path $ProjectRoot ".casa-backups\staging-full-prefix-16-to-30-v1r3-$Stamp"
    ...
    Write-Host "  approved data backfills:      migration 17 + migration 23 ONLY"
    Write-Host "  other top-level DML:           NONE"
    Write-Host "  destructive table/column DDL: NONE"
    ...
    if (
        [int]$Stage28.backfill.schoolsMissingBranch -ne 0 -or
        [int]$Stage28.backfill.sectionsMissingBranchMapping -ne 0 -or
        [int]$Stage28.backfill.classArmsMissingBranchMapping -ne 0 -or
        [int]$Stage28.backfill.terminalsMissingBranchMapping -ne 0
    ) {
        Stop-Phase "Migration 17 real-world backfill is incomplete on Staging."
    }

    if (
        [int]$Stage28.backfill.studentsMissingArrivalAssignment -ne 0 -or
        [int]$Stage28.backfill.arrivalAssignments -lt
            [int]$Stage28.backfill.students
    ) {
        Stop-Phase "Migration 23 arrival-method backfill is incomplete on Staging."
    }

    if ([int]$Stage28.backfill.casaInternalMemberships -ne 0) {
        Stop-Phase "Migration 23 unexpectedly manufactured CASA internal memberships on Staging."
    }

    Write-Host "  Staging migrations:           28 / EXACT"
    Write-Host "  migration-17 Main Campus:      ALL SCHOOLS COVERED"
    Write-Host "  section/class/terminal mapping: COMPLETE"
    Write-Host "  migration-23 arrival defaults: ALL STUDENTS COVERED"
    Write-Host "  migration-23 CASA internals:   0 / NONE MANUFACTURED"
    ...
    $CardStatusesBefore30 = $Stage29.identityCardStatusCounts
    ...
    foreach ($Property in $CardStatusesBefore30.PSObject.Properties) {
        $Status = [string]$Property.Name
        $BeforeCount = [int]$Property.Value
        $AfterProperty =
            $StagingAfter.identityCardStatusCounts.PSObject.Properties[$Status]

        if ($null -eq $AfterProperty) {
            Stop-Phase "Migration 30 removed existing student identity-card status '$Status'."
        }

        $AfterCount =
            [int]$AfterProperty.Value

        if ($AfterCount -ne $BeforeCount) {
            Stop-Phase "Migration 30 rewrote existing student identity-card status '$Status': before=$BeforeCount after=$AfterCount."
        }
    }

    $ReadyAfterProperty =
        $StagingAfter.identityCardStatusCounts.PSObject.Properties["READY_FOR_ACTIVATION"]

    if (
        $null -ne $ReadyAfterProperty -and
        [int]$ReadyAfterProperty.Value -ne 0
    ) {
        Stop-Phase "Migration 30 unexpectedly backfilled existing identity cards to READY_FOR_ACTIVATION."
    }
    ...
    $Result = [ordered]@{
        ...
        proof = "CASA_STAGING_FULL_PREFIX_16_TO_30_V1R3"
        ...
        approvedIntentionalMigrationBackfills = @("17_MAIN_CAMPUS_AND_MAPPINGS","23_DEFAULT_INDEPENDENT_ARRIVAL")
        unexpectedTopLevelMigrationDmlDetected = $false
        destructiveTableColumnRemovalDetected = $false
        migration30ExistingCardStatusesPreserved = $true
        migration30HistoricalArrivalActorsPreserved = $true
        passAEnums = "VERIFIED"
    ...
    Write-Host ""
    Write-Host "CASA STAGING FULL-PREFIX PROMOTION 16 -> 30 V1R3 IS GREEN" -ForegroundColor Green
    ...
    Write-Host "  approved migration backfills:     17 + 23 / VERIFIED"
    Write-Host "  unexpected top-level migration DML: NONE"
    Write-Host "  migration-30 existing card rows:   STATUS VALUES UNCHANGED"
    ...
    Write-Host ""
    Write-Host "CASA STAGING FULL-PREFIX PROMOTION V1R3 STOPPED: $($_.Exception.Message)" -ForegroundColor Red
