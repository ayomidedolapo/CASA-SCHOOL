...
$ExpectedRenewalHash =
    "458982ebd6076b28777ad14aad1d66c9dd960e1420e5a42b426cc87867997519"
...
Write-Host "CASA School - Staging Runtime + Functional Safe Smoke Verification V1R3" -ForegroundColor Green
Write-Host "Verifies the exact migration-30 source against Staging through a local Next.js runtime."
Write-Host "V1R3 keeps the corrected renewal-production.ts authority and fixes the Scanner manifest smoke URL from /manifest.webmanifest to the actual /scanner/manifest.webmanifest route; all database and Production safety guards are unchanged."
...
try {
    ...
    $Evidence =
        Join-Path `
            $ProjectRoot `
            ".casa-backups\staging-runtime-functional-smoke-v1r3-$Stamp"
...
'@
    ...
    $Manifest =
        Find-Check "/scanner/manifest.webmanifest"
    ...
    if ($null -eq $Manifest -or [int]$Manifest.status -ne 200) {
        Stop-Phase "/scanner/manifest.webmanifest did not return HTTP 200."
    }
    ...
    Write-Host "  /scanner/manifest.webmanifest: HTTP 200"
    ...
    $Result =
        [ordered]@{
            createdAt =
                (Get-Date -Format o)
            proof =
                "CASA_STAGING_RUNTIME_FUNCTIONAL_SAFE_SMOKE_V1R3"
            classification =
                "GREEN"
            sourceAuthority =
                "main/a7aa668"
    ...
    Write-Host ""
    Write-Host "CASA STAGING RUNTIME + FUNCTIONAL SAFE SMOKE V1R3 IS GREEN" -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "CASA STAGING RUNTIME + FUNCTIONAL SAFE SMOKE V1R3 STOPPED: $($_.Exception.Message)" -ForegroundColor Red
