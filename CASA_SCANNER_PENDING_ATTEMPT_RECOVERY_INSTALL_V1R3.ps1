...
Write-Host "CASA School - Scanner Pending Attempt Recovery Install V1R2" -ForegroundColor Green
Write-Host "Installs safe reload recovery for the exact pending Scanner attempt; source only."
Write-Host "V1R2 keeps the exact V1R1 recovery patch and hardens only read-only Neon probes with bounded retry/error capture."
...
try {
  ...
  $Evidence = Join-Path $ProjectRoot ".casa-backups\scanner-pending-attempt-recovery-install-v1r2-$stamp"
...
'@
  ...
  $proof = [ordered]@{
    ...
    proof = "CASA_SCANNER_PENDING_ATTEMPT_RECOVERY_INSTALL_V1R2"
