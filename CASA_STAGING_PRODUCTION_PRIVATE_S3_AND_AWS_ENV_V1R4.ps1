...
'@
    ...
    if (
        $PublicAccessBytes.Length -ge 3 -and
        $PublicAccessBytes[0] -eq 0xEF -and
        $PublicAccessBytes[1] -eq 0xBB -and
        $PublicAccessBytes[2] -eq 0xBF
    ) {
        Fail "Public Access Block JSON unexpectedly contains a UTF-8 BOM."
    }
...
'@
    ...
    if (
        $EncryptionBytes.Length -ge 3 -and
        $EncryptionBytes[0] -eq 0xEF -and
        $EncryptionBytes[1] -eq 0xBB -and
        $EncryptionBytes[2] -eq 0xBF
    ) {
        Fail "Encryption JSON unexpectedly contains a UTF-8 BOM."
    }
...
'@
    ...
    if (
        $OwnershipBytes.Length -ge 3 -and
        $OwnershipBytes[0] -eq 0xEF -and
        $OwnershipBytes[1] -eq 0xBB -and
        $OwnershipBytes[2] -eq 0xBF
    ) {
        Fail "Ownership JSON unexpectedly contains a UTF-8 BOM."
    }
...
"@
    ...
    if (
        $TagsBytes.Length -ge 3 -and
        $TagsBytes[0] -eq 0xEF -and
        $TagsBytes[1] -eq 0xBB -and
        $TagsBytes[2] -eq 0xBF
    ) {
        Fail "Tags JSON unexpectedly contains a UTF-8 BOM."
    }
...
"@
    ...
    if (
        $PolicyBytes.Length -ge 3 -and
        $PolicyBytes[0] -eq 0xEF -and
        $PolicyBytes[1] -eq 0xBB -and
        $PolicyBytes[2] -eq 0xBF
    ) {
        Fail "TLS policy JSON unexpectedly contains a UTF-8 BOM."
    }
...
Write-Host "CASA School - Staging + Production Private S3 and AWS Environment Scaffold V1R4" -ForegroundColor Green
...
try {
    ...
    Write-Host "  read-only AWS wrapper preflight: START"
    ...
    $preflightIdentity =
        Invoke-AwsJson `
            ...
            "read-only AWS wrapper preflight"

    if (
        $null -eq $preflightIdentity -or
        [string]::IsNullOrWhiteSpace(
            [string]$preflightIdentity.Account
        )
    ) {
        Fail "AWS wrapper preflight did not return a valid caller identity."
    }

    Write-Host "  read-only AWS wrapper preflight: PASS"
    ...
    $Evidence =
        Join-Path `
            $ProjectRoot `
            ".casa-backups\staging-production-s3-env-v1r4-$stamp"
    ...
    $result =
        [ordered]@{
            createdAt =
                (Get-Date -Format o)
            proof =
                "CASA_STAGING_PRODUCTION_PRIVATE_S3_ENV_V1R4"
            branch =
                $branch
            head =
                $head
    ...
    Write-Host ""
    Write-Host "CASA STAGING + PRODUCTION PRIVATE S3 AND AWS ENVIRONMENT SCAFFOLD V1R4 IS GREEN" -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "CASA STAGING + PRODUCTION PRIVATE S3 AND AWS ENVIRONMENT SCAFFOLD V1R4 STOPPED: $($_.Exception.Message)" -ForegroundColor Red
