...
try {
    Write-Host "CASA School - Scanner AWS Credential Chain Recovery V1" -ForegroundColor Green
    ...
    $EvidenceRoot = Join-Path $ProjectRoot ".casa-backups\scanner-aws-credential-chain-recovery-v1-$stamp"
    ...
    if ($afterProcess -ne $desiredProcess) {
        Stop-Recovery "credential_process did not persist the exact absolute-path bridge."
    }
    ...
    $sessionName = "casa-credential-recovery-" + (Get-Date -Format "yyyyMMddHHmmss")
    $stream = Invoke-AwsCapture $AwsExe @("sts","assume-role","--role-arn",$ExpectedStreamRoleArn,"--role-session-name",$sessionName,"--duration-seconds","900","--policy",$policy,"--profile",$BackendProfile,"--region",$ExpectedRegion,"--query","Credentials.Expiration","--output","text","--no-cli-pager")
    if ($stream.ExitCode -ne 0) {
        Stop-Recovery "Backend role cannot assume the liveness streaming role: $(Aws-Summary $stream)"
    }
    Write-Host "  streaming-role AssumeRole:   VERIFIED / TEMPORARY ONLY"
...
console.log(JSON.stringify({ callerVerified: true, rekognitionVerified: true }));
'@
...
'@
    ...
    $result = [ordered]@{
        ...
        proof = "CASA_SCANNER_AWS_CREDENTIAL_CHAIN_RECOVERY_V1"
    ...
    Write-Host ""
    Write-Host "CASA SCANNER AWS CREDENTIAL CHAIN RECOVERY V1 IS GREEN" -ForegroundColor Green
