...
function Assert-NoDatabaseUrlEcho(
    [string]$DatabaseUrl,
    [string]$Output,
    [string]$Label
) {
    if (
        ...
        $Output.Contains($DatabaseUrl)
    ) {
        Fail "$Label unexpectedly echoed DATABASE_URL. Output suppressed."
    }
}
...
function Add-ManagedVariable(
    ...
    Assert-NoDatabaseUrlEcho `
        $DatabaseUrl `
        $combined `
        "Vercel add $Environment/$Name"
}

Write-Host "CASA School - Final Vercel Operational-Staging Environment Binding V1R4 Recovery" -ForegroundColor Green
Write-Host "Recovers V1/V1R2 readiness failures and V1R3 partial Preview binding after an over-broad secret-echo guard false positive."
...
try {
    ...
    Write-Host "  V1/V1R2 remote mutation:        NONE"`n    Write-Host "  V1R3 remote state:              PARTIAL PREVIEW BINDING / WILL CONVERGE"
    ...
    $evidence =
        Join-Path `
            $ProjectRoot `
            ".casa-backups\final-vercel-operational-staging-binding-v1r4-$stamp"
    ...
    foreach ($key in $AllManagedKeys) {
        ...
        Add-ManagedVariable `
            ...
            ([string]$previewValues["DATABASE_URL"])
    ...
    foreach ($key in $AllManagedKeys) {
        ...
        Add-ManagedVariable `
            ...
            ([string]$productionTargetValues["DATABASE_URL"])
    ...
    $result =
        [ordered]@{
            createdAt =
                (Get-Date -Format o)
            proof =
                "CASA_FINAL_VERCEL_OPERATIONAL_STAGING_ENVIRONMENT_BINDING_V1R4"
            sourceAuthority =
                "$branch/$head"
            migrations =
                $migrationCount
            recoveredFrom =
                "V1_V1R2_NO_MUTATION__V1R3_PARTIAL_PREVIEW_BINDING_FALSE_POSITIVE_SECRET_GUARD"
            vercel = [ordered]@{
                teamSlug =
                    $TeamSlug
                teamId =
    ...
    Write-Host ""
    Write-Host "CASA FINAL VERCEL OPERATIONAL-STAGING ENVIRONMENT BINDING V1R4 IS GREEN" -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "CASA FINAL VERCEL OPERATIONAL-STAGING ENVIRONMENT BINDING V1R4 STOPPED: $($_.Exception.Message)" -ForegroundColor Red
    Write-Host "V1R4 is convergent for the managed Preview/Production-target keys. Do not manually edit them after a partial run; return the complete output and we can safely reconcile the exact state."
    throw
}