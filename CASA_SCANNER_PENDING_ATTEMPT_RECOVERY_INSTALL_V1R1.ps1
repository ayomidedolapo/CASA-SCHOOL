...
Write-Host "CASA School - Scanner Pending Attempt Recovery Install V1R1" -ForegroundColor Green
Write-Host "Installs safe reload recovery for the exact pending Scanner attempt; source only."
Write-Host "V1R1 narrows stranded-session verification to startAwsVerificationLiveness only; unrelated liveness paths remain untouched."
...
try {
  ...
  $Evidence = Join-Path $ProjectRoot ".casa-backups\scanner-pending-attempt-recovery-install-v1r1-$stamp"
