...
Write-Host "CASA School - Post-Face Success Authoritative Verification V1" -ForegroundColor Green
Write-Host "READ ONLY. Verifies the successful Scanner face/liveness ceremony after the previous HTTP Neon postflight transport failure."
...
try {
    ...
    $Evidence =
        Join-Path `
            $ProjectRoot `
            ".casa-backups\scanner-post-face-authoritative-verification-v1-$Stamp"
    ...
    if (
        ...
        (Compare-Object $ExpectedAttemptIds $ObservedIds).Count -ne 0
    ) {
        Write-Host ""
    ...
    if (
        ...
        $Result =
            [ordered]@{
                ...
                proof = "CASA_SCANNER_POST_FACE_SUCCESS_AUTHORITATIVE_VERIFICATION_V1"
    ...
    Write-Host ""
    Write-Host "CASA POST-FACE SUCCESS AUTHORITATIVE VERIFICATION STOPPED: $($_.Exception.Message)" -ForegroundColor Red
