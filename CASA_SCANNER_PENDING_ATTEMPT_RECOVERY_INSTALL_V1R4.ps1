...
function Run-DbProbe([string]$Label) {
  ...
  try {
    ...
    for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
      $stdoutPath = Join-Path $Evidence ("db-probe-{0}-{1}.stdout.log" -f (($Label -replace '[^A-Za-z0-9_-]','-').ToLowerInvariant()), $attempt)
      $stderrPath = Join-Path $Evidence ("db-probe-{0}-{1}.stderr.log" -f (($Label -replace '[^A-Za-z0-9_-]','-').ToLowerInvariant()), $attempt)

      Remove-Item -LiteralPath $stdoutPath,$stderrPath -Force -ErrorAction SilentlyContinue

      $proc = Start-Process `
        -FilePath "node.exe" `
        -ArgumentList @("--env-file=.env.local", "`"$DbProbePath`"") `
      ...
      if ($proc.ExitCode -eq 0) {
        ...
        if ($lines.Count -lt 1) {
          Stop-Phase "$Label Development read-only probe returned no JSON output."
        }
        try {
          return ($lines[-1] | ConvertFrom-Json)
        }
        catch {
          Write-Host "  $Label probe JSON parse failure:" -ForegroundColor Red
          $stdout | ForEach-Object { Write-Host "    $_" }
          Stop-Phase "$Label Development read-only probe returned invalid JSON."
        }
      }

      $lastFailure = @($stderr + $stdout | ForEach-Object { [string]$_ } | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
      Write-Host "  $Label Development read attempt ${attempt}/${maxAttempts}: FAILED" -ForegroundColor Yellow
    ...
    Write-Host "  $Label final Neon error:" -ForegroundColor Red
    if ($lastFailure.Count -gt 0) {
      $lastFailure | Select-Object -Last 20 | ForEach-Object { Write-Host "    $_" }
    }
    else {
      Write-Host "    Node exited non-zero without captured stderr/stdout."
    }
    Stop-Phase "$Label Development read-only probe failed after $maxAttempts attempts; no source write was authorized."
  }
  finally {
...
Write-Host "CASA School - Scanner Pending Attempt Recovery Install V1R4" -ForegroundColor Green
Write-Host "Installs safe reload recovery for the exact pending Scanner attempt; source only."
Write-Host "V1R4 keeps the same recovery semantics, retries read-only Neon probes, and anchors Scanner retry patching to the exact liveness-start URL instead of generic terminalFetch counts."
Write-Host "Adds replay-safe liveness-start semantics so a lost response cannot strand an active AWS session."
Write-Host "V1R4 patch guard: exact liveness-start URL anchor; unrelated terminalFetch calls remain untouched."
...
try {
  ...
  $Evidence = Join-Path $ProjectRoot ".casa-backups\scanner-pending-attempt-recovery-install-v1r4-$stamp"
...
const verificationFnEnd = aws.indexOf("export async function completeAwsVerificationLiveness", fnStart);
if (verificationFnEnd < 0) fail("completeAwsVerificationLiveness boundary not found after verification-start patch");
const verificationStartAfter = aws.slice(fnStart, verificationFnEnd);
if (verificationStartAfter.includes('"LIVENESS_SESSION_ALREADY_ACTIVE"')) fail("verification liveness-start path still strands an active session behind 409");
for (const marker of ["LIVENESS_SESSION_EXPIRED","activeSession.providerSessionId","replayed:","lte("]) if (!verificationStartAfter.includes(marker)) fail(`verification liveness-start postcondition missing ${marker}`);
write(awsPath,aws);
...
const livenessStartUrl = "`/api/terminal/attempts/${attemptId}/biometric/liveness/start`";
const livenessStartUrlCount = startFace.split(livenessStartUrl).length - 1;
if (livenessStartUrlCount !== 1) fail(`startFace liveness-start URL shape drifted (${livenessStartUrlCount})`);
const livenessUrlIndex = startFace.indexOf(livenessStartUrl);
const terminalCall = "await terminalFetch(";
const livenessCallIndex = startFace.lastIndexOf(terminalCall,livenessUrlIndex);
if (livenessCallIndex < 0) fail("startFace liveness-start terminalFetch call not found");
const livenessCallPrefix = startFace.slice(livenessCallIndex,livenessUrlIndex);
if (!livenessCallPrefix.includes("token")) fail("startFace liveness-start call no longer carries terminal token before URL");
startFace =
  startFace.slice(0,livenessCallIndex) +
  "await terminalFetchWithRetry(" +
  startFace.slice(livenessCallIndex + terminalCall.length);
const patchedUrlIndex = startFace.indexOf(livenessStartUrl);
const retryCallIndex = startFace.lastIndexOf("await terminalFetchWithRetry(",patchedUrlIndex);
if (retryCallIndex < 0) fail("startFace liveness-start retry call was not installed");
...
'@
  ...
  $proof = [ordered]@{
    ...
    proof = "CASA_SCANNER_PENDING_ATTEMPT_RECOVERY_INSTALL_V1R4"
