...
function Invoke-NodeExpression(
    [string]$Expression,
    [string]$Label
) {
    $node =
...
Write-Host "CASA School - Vercel OIDC AWS Runtime Source Wiring V1R3 Recovery" -ForegroundColor Green
...
try {
    ...
    # npm generated these files. Use Node's JSON.parse/require semantics as the
    # authority rather than PowerShell ConvertFrom-Json over a large lockfile.
    $packageDependencyExpression =
        "const p=require('./package.json'); const v=p.dependencies?.['$OidcPackage']; if(!v){process.exit(2)}; process.stdout.write(String(v));"

    $packageDependencyVersion =
        Invoke-NodeExpression `
            $packageDependencyExpression `
            "package.json OIDC dependency verification"

    if (
        $packageDependencyVersion -ne
        $OidcPackageVersion
    ) {
        Fail "V1 partial state is not exact: package.json has $OidcPackage=$packageDependencyVersion, expected $OidcPackageVersion."
    }

    $lockVersionExpression =
        "const p=require('./package-lock.json'); const v=p.packages?.['node_modules/$OidcPackage']?.version; if(!v){process.exit(2)}; process.stdout.write(String(v));"

    $packageLockVersion =
        Invoke-NodeExpression `
            $lockVersionExpression `
            "package-lock.json OIDC dependency verification"

    if (
        $packageLockVersion -ne
        $OidcPackageVersion
    ) {
        Fail "package-lock.json locks $OidcPackage=$packageLockVersion, expected $OidcPackageVersion."
    }
    ...
    $installedVersionExpression =
        "const p=require('./node_modules/$OidcPackage/package.json'); if(!p.version){process.exit(2)}; process.stdout.write(String(p.version));"

    $installedVersion =
        Invoke-NodeExpression `
            $installedVersionExpression `
            "installed OIDC package verification"

    if (
        $installedVersion -ne
        $OidcPackageVersion
    ) {
        Fail "Installed OIDC package version drift: $installedVersion"
    }

    Write-Host "  package.json dependency:        $OidcPackageVersion / VERIFIED BY NODE"
    Write-Host "  package-lock version:           $OidcPackageVersion / VERIFIED BY NODE"
    Write-Host "  node_modules version:           $OidcPackageVersion / VERIFIED BY NODE"
...
'@
    ...
    $backup =
        Join-Path `
            $ProjectRoot `
            ".casa-backups\vercel-oidc-aws-runtime-source-wiring-v1r3-recovery-$stamp"
    ...
    $result =
        [ordered]@{
            createdAt =
                (Get-Date -Format o)
            proof =
                "CASA_VERCEL_OIDC_AWS_RUNTIME_SOURCE_WIRING_V1R3_RECOVERY"
    ...
    Write-Host ""
    Write-Host "CASA VERCEL OIDC AWS RUNTIME SOURCE WIRING V1R3 RECOVERY IS GREEN" -ForegroundColor Green
    ...
    Write-Host "  npm install in V1R3:            NONE"
    ...
    Write-Host ""
    Write-Host "CASA VERCEL OIDC AWS RUNTIME SOURCE WIRING V1R3 RECOVERY STOPPED: $($_.Exception.Message)" -ForegroundColor Red
