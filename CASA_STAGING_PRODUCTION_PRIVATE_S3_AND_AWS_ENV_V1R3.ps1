...
Write-Host "CASA School - Staging + Production Private S3 and AWS Environment Scaffold V1R3" -ForegroundColor Green
...
try {
    ...
    $Evidence =
        Join-Path `
            $ProjectRoot `
            ".casa-backups\staging-production-s3-env-v1r3-$stamp"
    ...
    $result =
        [ordered]@{
            createdAt =
                (Get-Date -Format o)
            proof =
                "CASA_STAGING_PRODUCTION_PRIVATE_S3_ENV_V1R3"
            branch =
                $branch
            head =
                $head
    ...
    Write-Host ""
    Write-Host "CASA STAGING + PRODUCTION PRIVATE S3 AND AWS ENVIRONMENT SCAFFOLD V1R3 IS GREEN" -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "CASA STAGING + PRODUCTION PRIVATE S3 AND AWS ENVIRONMENT SCAFFOLD V1R3 STOPPED: $($_.Exception.Message)" -ForegroundColor Red
