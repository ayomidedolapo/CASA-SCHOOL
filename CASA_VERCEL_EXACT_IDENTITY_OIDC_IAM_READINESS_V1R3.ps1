...
function Invoke-Vercel(
    ...
    if (
        [string]::IsNullOrWhiteSpace(
            [string]$script:VercelRunnerCommand
        )
    ) {
...
Write-Host "CASA School - Vercel Exact Identity + OIDC IAM Binding Readiness V1R3" -ForegroundColor Green
...
Write-Host "Uses installed Vercel CLI when available, otherwise transient npx vercel@latest. No Vercel project creation/link mutation."
Write-Host "No AWS IAM mutation. No S3 mutation. No DB connection. No migration. No deployment."
...
try {
    ...
    if ($null -ne $vercelCommand) {
        ...
        if ($null -eq $npxCommand) {
            Fail "Vercel CLI is not installed and npx is not available. Install Node/npm or Vercel CLI before continuing."
        }

        $script:VercelRunnerCommand =
            [string]$npxCommand.Source
    ...
    if (-not $whoami.ok) {
        ...
        Fail "Vercel CLI is available but not authenticated. Run '$loginCommand', complete browser authentication, then rerun this V1R3 probe."
    }
    ...
    $result =
        [ordered]@{
            ...
            proof = "CASA_VERCEL_OIDC_EXACT_IDENTITY_READINESS_V1R3"
            source = [ordered]@{
                branch = $branch
                head = $head
    ...
    Write-Host ""
    Write-Host "CASA VERCEL EXACT IDENTITY + OIDC IAM BINDING READINESS V1R3 IS GREEN" -ForegroundColor Green
    Write-Host "  source authority:               $branch/$head"
    Write-Host "  AWS account/region:             $ExpectedAwsAccount / $AwsRegion"
    Write-Host "  Vercel CLI runner:              $script:VercelRunnerMode"
    ...
    Write-Host ""
    Write-Host "CASA VERCEL EXACT IDENTITY + OIDC IAM BINDING READINESS V1R3 STOPPED: $($_.Exception.Message)" -ForegroundColor Red
