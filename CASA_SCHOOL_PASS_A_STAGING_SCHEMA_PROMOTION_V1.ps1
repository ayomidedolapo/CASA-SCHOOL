...
function Invoke-Probe([string]$DatabaseUrl, [string]$ExpectedHost, [string]$Label) {
  ...
  try {
    $ErrorActionPreference = "Continue"
    $stdout = @(& node $script:ProbePath $script:RepoRoot $ExpectedHost $Label 2> $stderrPath)
    $exitCode = $LASTEXITCODE
  }
  finally { $ErrorActionPreference = $previous }
  $stderr = @()
  ...
  if ($exitCode -ne 0) {
    Abort "$Label read-only probe failed.`n$($stdout -join [Environment]::NewLine)`n$($stderr -join [Environment]::NewLine)"
  }
...
try {
  Write-Host "CASA School - Pass A Guarded Staging Schema Promotion V1"
  ...
  foreach ($relative in ($PayloadManifest.Keys | Sort-Object)) { Assert-Hash $relative ([string]$PayloadManifest[$relative]) }
  ...
  try {
    $ErrorActionPreference='Continue'
    & git -C $script:RepoRoot check-ignore --quiet -- '.env.staging.local'
  ...
  $script:EvidenceRoot=Join-Path (Repo-Path '.casa-backups') "pass-a-staging-promotion-$stamp"
  ...
  Write-Host ""
  Write-Host "PASS A STAGING SCHEMA PROMOTION PASSED."
  Write-Host "  source authority: main/a7aa668 + exact recovered Pass A postimages"
