...
]);const propagation=await refreshUnprintedCardsForTemplate({schoolId:target.school_id,templateId});return NextResponse.json({activated:true,propagation},{headers:casaInternalNoStoreHeaders})}catch(e){const r=casaInternalAuthErrorResponse(e);if(r)return r;console.error("Template activation failed",e);return NextResponse.json({message:"Template activation failed."},{status:500,headers:casaInternalNoStoreHeaders})}}
