...
export async function GET(request:NextRequest){
 try{
  ...
  const jobs=await listCentralProductionJobs({status:status?.success?status.data:null,schoolId:schoolId??null,branchId:branchId??null,limit,origin:request.nextUrl.origin});
