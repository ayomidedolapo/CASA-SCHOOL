...
export default async function InternalHomePage() {
  ...
  const health=rowsOf<{unread:number;offline:number}>(await db.execute(sql`
    select
      ...
      (select count(*)::int from attendance_terminals terminal join schools school on school.id=terminal.school_id where terminal.status='ACTIVE'::attendance_terminal_status and school.status='ACTIVE'::school_status and (terminal.last_seen_at is null or terminal.last_seen_at < now()-interval '5 minutes') and (${access.membership.role}='CASA_SUPER_ADMIN' or exists(select 1 from casa_internal_school_assignments a where a.membership_id=${access.membership.id}::uuid and a.school_id=terminal.school_id and a.status='ACTIVE'))) offline
  ...
  return <InternalShell actorName={access.session.fullName} role={access.membership.role} active="overview">
    ...
    <div className="px-5 py-6 sm:px-8 lg:px-10 lg:py-8">
      ...
      <Link href="/internal/notifications" className="mt-6 grid gap-3 border border-black bg-white p-5 transition hover:bg-[#f2f2ef] sm:grid-cols-[1fr_auto_auto] sm:items-center"><div><p className="casa-kicker text-black/40">Operational notifications</p><p className="mt-2 text-sm text-black/55">Scanner state uses the current authenticated heartbeat. A scanner is counted as not responding for more than five minutes without a heartbeat.</p></div><span className="text-sm"><strong>{health.unread}</strong> unread</span><span className="text-sm"><strong>{health.offline}</strong> scanner{health.offline===1?"":"s"} not responding</span></Link>
