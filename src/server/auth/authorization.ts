...
export async function requireSchoolAccess(
  ...
  return withTransientDatabaseReadRetry(async () => {
    const memberships = await db
      ...
      .innerJoin(
        ...
        and(
          ...
          eq(schoolMemberships.userId, session.userId),
        ),
      )
      .where(
        and(
    ...
    if (!membership) throw new SchoolAccessDeniedError();

    const roleRows = await db
      .select({ role: schoolMembershipRoles.role })
      .from(schoolMembershipRoles)
      .where(
        and(
          eq(schoolMembershipRoles.schoolId, membership.schoolId),
          eq(schoolMembershipRoles.membershipId, membership.membershipId),
        ),
      );
...
export async function requireSchoolRole(
  ...
  const allowed = access.roles.some((role) => allowedRoles.includes(role));
  if (!allowed) throw new SchoolAccessDeniedError();
  return access;
}
