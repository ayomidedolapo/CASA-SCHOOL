...
try {
  Write-Host "CASA School - Permanent Card + Scanner Simplification V1R7" -ForegroundColor Green
  ...
  $scannerManifestPath = Join-Path $ProjectRoot "src\app\scanner\manifest.ts"
  $staticScannerManifestPath = Join-Path $ProjectRoot "public\scanner\manifest.webmanifest"

  foreach ($requiredPath in @($scannerManifestPath, $staticScannerManifestPath)) {
    if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
      Stop-Phase "Required Scanner manifest missing: $requiredPath"
    }
  }

  $scannerManifestBefore = Get-Content -LiteralPath $scannerManifestPath -Raw
  $staticScannerManifestBefore = Get-Content -LiteralPath $staticScannerManifestPath -Raw
  foreach ($pair in @(
    @{ Text = $scannerManifestBefore; Label = "src/app/scanner/manifest.ts" },
    @{ Text = $staticScannerManifestBefore; Label = "public/scanner/manifest.webmanifest" }
  )) {
    if (-not $pair.Text.Contains("CASA School Scanner")) {
      Stop-Phase "$($pair.Label) no longer contains the expected pre-patch CASA School Scanner brand."
    }
    if (-not $pair.Text.Contains("CASA School identity-verified attendance scanner.")) {
      Stop-Phase "$($pair.Label) no longer contains the expected pre-patch Scanner description."
    }
    if (-not $pair.Text.Contains("CASA Scanner") -or -not $pair.Text.Contains("/scanner")) {
      Stop-Phase "$($pair.Label) lost required Scanner PWA identity/scope markers."
    }
  }
  ...
  Write-Host "  Scanner PWA pre-brand state:    CASA SCHOOL -> CASA / VERIFIED"
  Write-Host "  AWS liveness source:            BYTE-FOR-BYTE LOCKED"

  Step "Creating guarded source backup"
  $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
  $Evidence = Join-Path $ProjectRoot ".casa-backups\permanent-card-scanner-simplification-v1r7-$stamp"
  ...
  [void]$targets.Add($scannerManifestPath)
  [void]$targets.Add($staticScannerManifestPath)
  ...
  $PatcherPath = Join-Path $Evidence "apply-permanent-card-scanner-v1r7.mjs"
  $patcher = @'
...
//    The policy predicate is explicitly typed boolean instead of a compile-time
//    literal so TypeScript continues to analyze/narrow the preserved historical
//    implementation below the runtime gate.
{
  let source = read("src/server/card-production/renewal-production.ts");
  const functionAnchor = "export async function produceStudentCardRenewalItem(";
  if (!source.includes(functionAnchor)) fail("renewal-production: item entrypoint anchor missing");
  if (source.includes("function routineCardRenewalDisabled(): boolean")) {
    fail("renewal-production: permanent-card policy helper already present");
  }
  const helper = `function routineCardRenewalDisabled(): boolean {
  return true;
}
...
`;
  ...
  if ((source.match(/routineCardRenewalDisabled\(\)/g) ?? []).length !== 3) {
    fail("renewal-production: expected one policy helper and two guarded entrypoints");
  }
...
// 8) Scanner PWA installation identity uses CASA as the product brand.
//    School and terminal names remain live/dynamic screen context, not a second app brand.
{
  let source = read("src/app/scanner/manifest.ts");
  ...
  source = replaceUnique(
    ...
    "scanner manifest description"
  );
  if (source.includes("CASA School")) fail("scanner manifest: CASA School remains as a separate product brand");
  if (!source.includes('"CASA Scanner"') || !source.includes('"/scanner"')) {
    fail("scanner manifest: CASA Scanner identity/scope missing after rebrand");
  }
  write("src/app/scanner/manifest.ts", source);

  let staticManifest = read("public/scanner/manifest.webmanifest");
  staticManifest = replaceUnique(
    staticManifest,
    '"name": "CASA School Scanner"',
    '"name": "CASA Scanner"',
    "static scanner manifest name"
  );
  staticManifest = replaceUnique(
    staticManifest,
    '"description": "CASA School identity-verified attendance scanner."',
    '"description": "CASA identity-verified attendance scanner."',
    "static scanner manifest description"
  );
  if (staticManifest.includes("CASA School")) fail("static scanner manifest: CASA School remains as a separate product brand");
  if (!staticManifest.includes('"name": "CASA Scanner"') || !staticManifest.includes('"start_url": "/scanner"')) {
    fail("static scanner manifest: CASA Scanner identity/scope missing after rebrand");
  }
  write("public/scanner/manifest.webmanifest", staticManifest);
}

// 9) Pass A source contract now asserts the permanent-card model.
{
...
// 10) Synchronize intentional permanent-card + Scanner changes with the historical frontend regression.
//     The historical frontend-closure test still owns every non-Scanner frontend contract.
//     Scanner UI/trust is now owned by scanner:selftest + this patch's explicit Scanner checks,
//     because the user intentionally retired the old installed-state badge and 01/02/03/04 rail.
{
  ...
  if (fs.existsSync(full(relative))) {
    ...
    const hashCount = source.split(oldHash).length - 1;
    if (hashCount > 1) fail(`frontend closure selftest contains ${hashCount} current Scanner hash copies; refusing broad rewrite`);
    if (hashCount === 1) {
      source = source.replace(oldHash, newHash);
    }

    const legacyRegistryMarker = "Reissue with Passkey";
    const permanentRegistryMarker = "Replace card with Passkey";
    const legacyCount = source.split(legacyRegistryMarker).length - 1;
    const permanentCount = source.split(permanentRegistryMarker).length - 1;
    if (legacyCount > 1 || permanentCount > 1) {
      fail(`frontend closure Registry marker multiplicity is unexpected: legacy=${legacyCount} permanent=${permanentCount}`);
    }
    if (legacyCount === 1 && permanentCount === 0) {
      source = source.replace(legacyRegistryMarker, permanentRegistryMarker);
    } else if (!(legacyCount === 0 && permanentCount === 1)) {
      fail(`frontend closure Registry marker state is unexpected: legacy=${legacyCount} permanent=${permanentCount}`);
    }

    if (!source.includes("Card action reason")) {
      fail("frontend closure lost the still-valid Card action reason contract");
    }

    const delegationMarker = "SCANNER_UI_CONTRACT_DELEGATED_TO_SCANNER_SELFTEST";
    if (!source.includes(delegationMarker)) {
      const assertStart = source.indexOf("function assert(");
      const expectMarkersStart = source.indexOf("function expectMarkers(", assertStart);
      if (assertStart < 0 || expectMarkersStart <= assertStart) {
        fail("frontend closure assert/expectMarkers boundary missing; refusing broad test rewrite");
      }
      const assertBlock = source.slice(assertStart, expectMarkersStart);
      if (!assertBlock.includes("throw new Error") || !assertBlock.includes("message")) {
        fail("frontend closure assert helper shape drifted; refusing Scanner-contract delegation");
      }
      const replacementAssert = `// ${delegationMarker}
...
`;
      source =
        source.slice(0, assertStart) +
        replacementAssert +
        source.slice(expectMarkersStart);
    }

    // Synchronize the two historical onboarding copy assertions with the
    // current functional contracts. The accepted onboarding source intentionally
    // no longer renders the standalone labels; behavior lives in these functions.
    const onboardingSource = read("src/app/internal/onboarding/onboarding-client.tsx");
    for (const required of [
      "async function chooseNextIncomplete",
      "async function releaseLock",
    ]) {
      if (!onboardingSource.includes(required)) {
        fail(`current onboarding source lost required functional contract: ${required}`);
      }
    }

    const arrow = "\u2192";
    const onboardingContractPairs = [
      {
        label: "Next-Incomplete",
        historical: `"Next incomplete ${arrow}"`,
        functional: '"async function chooseNextIncomplete"',
      },
      {
        label: "Release/Next-Incomplete",
        historical: `"Release & next incomplete ${arrow}"`,
        functional: '"async function releaseLock"',
      },
    ];

    for (const contract of onboardingContractPairs) {
      const historicalCount = source.split(contract.historical).length - 1;
      const functionalCount = source.split(contract.functional).length - 1;

      if (historicalCount > 1 || functionalCount > 1) {
        fail(`frontend closure ${contract.label} contract multiplicity unexpected: historical=${historicalCount} functional=${functionalCount}`);
      }

      if (historicalCount === 1 && functionalCount === 0) {
        source = source.replace(contract.historical, contract.functional);
      } else if (!(historicalCount === 0 && functionalCount === 1)) {
        fail(`frontend closure ${contract.label} contract state unexpected: historical=${historicalCount} functional=${functionalCount}`);
      }
    }
...
console.log(JSON.stringify({
  ...
  scannerManifestSha256: sha("src/app/scanner/manifest.ts"),
  staticScannerManifestSha256: sha("public/scanner/manifest.webmanifest")
}));
'@
  ...
  if (-not $renewalText.Contains("function routineCardRenewalDisabled(): boolean")) {
    Stop-Phase "Permanent-card runtime renewal policy helper is missing."
  }
  if (([regex]::Matches($renewalText, "routineCardRenewalDisabled\(\)")).Count -ne 3) {
    Stop-Phase "Permanent-card runtime renewal policy does not guard both legacy entrypoints."
  }
  ...
  $registryCardsText = Get-Content -LiteralPath (Join-Path $ProjectRoot "src\app\schools\[slug]\registry\student-cards.tsx") -Raw
  if (-not $registryCardsText.Contains("Card action reason")) {
    Stop-Phase "Exceptional card actions lost the required Card action reason field."
  }
  if (-not $registryCardsText.Contains("Replace card with Passkey") -or $registryCardsText.Contains("Reissue with Passkey")) {
    Stop-Phase "Registry permanent-card replacement wording is not synchronized."
  }

  if (Test-Path -LiteralPath $frontendSelftestPath -PathType Leaf) {
    $frontendSelftestText = Get-Content -LiteralPath $frontendSelftestPath -Raw
    if (-not $frontendSelftestText.Contains("Card action reason") -or -not $frontendSelftestText.Contains("Replace card with Passkey") -or $frontendSelftestText.Contains("Reissue with Passkey")) {
      Stop-Phase "Frontend closure permanent-card Registry contract is not synchronized."
    }
    if (-not $frontendSelftestText.Contains("SCANNER_UI_CONTRACT_DELEGATED_TO_SCANNER_SELFTEST")) {
      Stop-Phase "Frontend closure still owns obsolete Scanner UI assumptions instead of delegating to scanner:selftest."
    }

    $Arrow = [char]0x2192
    foreach ($obsoleteCopy in @(
      "Next incomplete $Arrow",
      "Release & next incomplete $Arrow"
    )) {
      if ($frontendSelftestText.Contains($obsoleteCopy)) {
        Stop-Phase "Frontend closure still contains stale onboarding copy contract: $obsoleteCopy"
      }
    }
    foreach ($functionalContract in @(
      ...
      if (-not $frontendSelftestText.Contains($functionalContract)) {
        Stop-Phase "Frontend closure lost current onboarding functional contract: $functionalContract"
      }
    }
  }
  ...
  $scannerManifestAfter = Get-Content -LiteralPath $scannerManifestPath -Raw
  $staticScannerManifestAfter = Get-Content -LiteralPath $staticScannerManifestPath -Raw
  foreach ($pair in @(
    @{ Text = $scannerManifestAfter; Label = "src/app/scanner/manifest.ts" },
    @{ Text = $staticScannerManifestAfter; Label = "public/scanner/manifest.webmanifest" }
  )) {
    if ($pair.Text.Contains("CASA School")) {
      Stop-Phase "$($pair.Label) still exposes CASA School as a separate product brand."
    }
    if (-not $pair.Text.Contains("CASA Scanner") -or -not $pair.Text.Contains("/scanner")) {
      Stop-Phase "$($pair.Label) lost the CASA Scanner identity/scope contract."
    }
  }
  ...
  Write-Host "  Scanner installed PWA brand:     CASA SCANNER / VERIFIED"
  Write-Host "  school + terminal names:         LIVE CONTEXT / NOT PRODUCT BRAND"
  ...
  Write-Host ""
  Write-Host "CASA PERMANENT CARD + SCANNER SIMPLIFICATION V1R7 IS GREEN" -ForegroundColor Green
  ...
  Write-Host "  installed Scanner PWA brand:     CASA SCANNER"
  Write-Host "  school/terminal naming:          DYNAMIC SCREEN CONTEXT"
