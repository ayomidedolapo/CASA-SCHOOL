...
Write-Host "CASA School - Vercel Exact Identity + OIDC IAM Binding Readiness V1" -ForegroundColor Green
...
try {
    ...
    $evidence =
        Join-Path `
            $ProjectRoot `
            ".casa-backups\vercel-oidc-exact-identity-readiness-v1-$stamp"
    ...
    $result =
        [ordered]@{
            ...
            proof = "CASA_VERCEL_OIDC_EXACT_IDENTITY_READINESS_V1"
            source = [ordered]@{
                branch = $branch
                head = $head
    ...
    Write-Host ""
    Write-Host "CASA VERCEL EXACT IDENTITY + OIDC IAM BINDING READINESS V1 IS GREEN" -ForegroundColor Green
    Write-Host "  source authority:               $branch/$head"
