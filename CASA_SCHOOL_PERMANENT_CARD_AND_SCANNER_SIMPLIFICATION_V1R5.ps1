...
try {
  Write-Host "CASA School - Permanent Card + Scanner Simplification V1R5" -ForegroundColor Green
  ...
  $Evidence = Join-Path $ProjectRoot ".casa-backups\permanent-card-scanner-simplification-v1r5-$stamp"
  ...
  $PatcherPath = Join-Path $Evidence "apply-permanent-card-scanner-v1r5.mjs"
  $patcher = @'
...
//     Keep the still-valid "Card action reason" contract; only Reissue -> Replace is a product change.
{
...
'@
  ...
  Write-Host ""
  Write-Host "CASA PERMANENT CARD + SCANNER SIMPLIFICATION V1R5 IS GREEN" -ForegroundColor Green
