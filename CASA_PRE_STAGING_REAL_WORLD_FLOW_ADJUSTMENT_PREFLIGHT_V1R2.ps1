...
try {
  Write-Host "CASA School - Pre-Staging Real-World Flow Adjustment Preflight V1R2" -ForegroundColor Green
  ...
  if($head-ne$ExpectedHead){Fail "Expected committed CASA checkpoint $ExpectedHead; found $head."}
  & git merge-base --is-ancestor $RequiredAncestor HEAD
  if($LASTEXITCODE-ne 0){Fail "Current checkpoint is not descended from required CASA authority $RequiredAncestor."}
  ...
  $report.Add("CASA PRE-STAGING REAL-WORLD FLOW ADJUSTMENT PREFLIGHT V1R2")
  $report.Add("created=$(Get-Date -Format o)"); $report.Add("branch=$branch"); $report.Add("HEAD=$head"); $report.Add("required_ancestor=$RequiredAncestor"); $report.Add("local_migrations=$($migrations.Count)")
  ...
  Write-Host ""
  Write-Host "CASA PRE-STAGING REAL-WORLD FLOW ADJUSTMENT PREFLIGHT V1R2 IS COMPLETE" -ForegroundColor Green
