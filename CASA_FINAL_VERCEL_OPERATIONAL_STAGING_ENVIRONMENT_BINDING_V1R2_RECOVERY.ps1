...
Write-Host "CASA School - Final Vercel Operational-Staging Environment Binding V1R2 Recovery" -ForegroundColor Green
Write-Host "Recovers V1 after Vercel CLI help printed valid syntax but returned exit code 2 before any mutation."
...
try {
    ...
    Step "Proving Vercel identity and parsing CLI capabilities without trusting help exit code"
    ...
    # Vercel CLI 59.15.1 prints valid subcommand help but may return exit code 2.
    # Help text itself is the authority here; no mutation can occur during help.
    $addHelp =
        Invoke-Vercel `
            @(
                "env",
    ...
    $addHelpText = Get-CombinedOutput $addHelp
    $rmHelpText = Get-CombinedOutput $rmHelp
    $lsHelpText = Get-CombinedOutput $lsHelp
    ...
    foreach ($required in @(
        ...
        if ($rmHelpText -notmatch [regex]::Escape($required)) {
            Fail "Current Vercel CLI env rm help does not prove required capability $required."
        }
    }

    if (
        $lsHelpText -notmatch [regex]::Escape("vercel env ls")
    ) {
        Fail "Current Vercel CLI env ls help is not recognized."
    }

    Write-Host "  authenticated Vercel user:      $ExpectedVercelUser / VERIFIED"
    Write-Host "  help non-zero exit behavior:    TOLERATED / TEXT VALIDATED"
    Write-Host "  env add --sensitive:            SUPPORTED"
    Write-Host "  env add --force:                SUPPORTED"
    Write-Host "  env add --yes:                  SUPPORTED"
    Write-Host "  preview/production targets:     SUPPORTED"
    Write-Host "  env rm --yes:                   SUPPORTED"
    Write-Host "  env ls:                         SUPPORTED"
    ...
    $evidence =
        Join-Path `
            $ProjectRoot `
            ".casa-backups\final-vercel-operational-staging-binding-v1r2-$stamp"
    ...
    $result =
        [ordered]@{
            createdAt =
                (Get-Date -Format o)
            proof =
                "CASA_FINAL_VERCEL_OPERATIONAL_STAGING_ENVIRONMENT_BINDING_V1R2"
            sourceAuthority =
                "$branch/$head"
            migrations =
                $migrationCount
            recoveredFrom =
                "V1_HELP_COMMAND_EXIT_2_BEFORE_REMOTE_MUTATION"
            vercel = [ordered]@{
                teamSlug =
                    $TeamSlug
                teamId =
    ...
    Write-Host ""
    Write-Host "CASA FINAL VERCEL OPERATIONAL-STAGING ENVIRONMENT BINDING V1R2 IS GREEN" -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "CASA FINAL VERCEL OPERATIONAL-STAGING ENVIRONMENT BINDING V1R2 STOPPED: $($_.Exception.Message)" -ForegroundColor Red
    Write-Host "V1R2 is convergent for the managed Preview/Production-target keys. Do not manually edit them after a partial run; return the complete output and we can safely reconcile the exact state."
    throw
}
