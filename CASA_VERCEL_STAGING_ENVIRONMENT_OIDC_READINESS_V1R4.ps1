...
Write-Host "CASA School - Vercel Staging Environment + OIDC Readiness V1R4" -ForegroundColor Green
...
try {
    ...
    # PowerShell unwraps single-item pipeline results by default. Every API
    # collection below is explicitly array-wrapped so StrictMode behaves
    # identically for zero, one, or many returned objects.
    $customEnvironments = @()
    ...
    # Keep the REST endpoint argument free of literal ampersands. On Windows,
    # npx.cmd can pass an unquoted `&` through cmd.exe as a command separator.
    # The exact team is scoped through Vercel's global --scope option instead.
    # The deployments endpoint defaults to a bounded recent page, which is
    # sufficient for proving the newly-created CASA School staging deployment.
    $deploymentsApiPath =
        "/v6/deployments" +
        "?projectId=$ExpectedProjectId"

    if ($deploymentsApiPath.Contains("&")) {
        Fail "Windows command-transport invariant failed: deployments API path contains an ampersand."
    }
    ...
    $evidence =
        Join-Path `
            $ProjectRoot `
            ".casa-backups\vercel-staging-environment-oidc-readiness-v1r4-$stamp"
    ...
    $result =
        [ordered]@{
            ...
            proof = "CASA_VERCEL_STAGING_ENVIRONMENT_OIDC_READINESS_V1R4"
            source = [ordered]@{
                branch = $branch
                head = $head
    ...
    Write-Host ""
    Write-Host "CASA VERCEL STAGING ENVIRONMENT + OIDC READINESS V1R4 IS GREEN" -ForegroundColor Green
    Write-Host "  source authority:               $branch/$head"
    Write-Host "  migrations:                     $migrationCount / VERIFIED"
    ...
    Write-Host ""
    Write-Host "CASA VERCEL STAGING ENVIRONMENT + OIDC READINESS V1R4 STOPPED: $($_.Exception.Message)" -ForegroundColor Red
