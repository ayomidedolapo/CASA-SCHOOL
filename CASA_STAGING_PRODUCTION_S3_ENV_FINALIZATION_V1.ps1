...
Write-Host "CASA School - Staging + Production S3 Environment Finalization V1" -ForegroundColor Green
...
try {
    ...
    $Evidence =
        Join-Path `
            $ProjectRoot `
            ".casa-backups\staging-production-s3-env-finalize-v1-$stamp"
    ...
    $result =
        [ordered]@{
            createdAt =
                (Get-Date -Format o)
            proof =
                "CASA_STAGING_PRODUCTION_S3_ENV_FINALIZATION_V1"
            branch =
                $branch
            head =
                $head
    ...
    Write-Host ""
    Write-Host "CASA STAGING + PRODUCTION S3 ENVIRONMENT FINALIZATION V1 IS GREEN" -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "CASA STAGING + PRODUCTION S3 ENVIRONMENT FINALIZATION V1 STOPPED: $($_.Exception.Message)" -ForegroundColor Red
