...
try {
  Write-Host "CASA School - Permanent Card + Scanner Simplification V1" -ForegroundColor Green
  ...
  $Evidence = Join-Path $ProjectRoot ".casa-backups\permanent-card-scanner-simplification-v1-$stamp"
  ...
  $PatcherPath = Join-Path $Evidence "apply-permanent-card-scanner-v1.mjs"
  $patcher = @'
...
'@
  ...
  Write-Host ""
  Write-Host "CASA PERMANENT CARD + SCANNER SIMPLIFICATION V1 IS GREEN" -ForegroundColor Green
