...
Write-Host "CASA School - Vercel Staging Environment + OIDC Readiness V1R3" -ForegroundColor Green
...
try {
    ...
    $evidence =
        Join-Path `
            $ProjectRoot `
            ".casa-backups\vercel-staging-environment-oidc-readiness-v1r3-$stamp"
    ...
    $result =
        [ordered]@{
            ...
            proof = "CASA_VERCEL_STAGING_ENVIRONMENT_OIDC_READINESS_V1R3"
            source = [ordered]@{
                branch = $branch
                head = $head
    ...
    Write-Host ""
    Write-Host "CASA VERCEL STAGING ENVIRONMENT + OIDC READINESS V1R3 IS GREEN" -ForegroundColor Green
    Write-Host "  source authority:               $branch/$head"
    Write-Host "  migrations:                     $migrationCount / VERIFIED"
    ...
    Write-Host ""
    Write-Host "CASA VERCEL STAGING ENVIRONMENT + OIDC READINESS V1R3 STOPPED: $($_.Exception.Message)" -ForegroundColor Red
