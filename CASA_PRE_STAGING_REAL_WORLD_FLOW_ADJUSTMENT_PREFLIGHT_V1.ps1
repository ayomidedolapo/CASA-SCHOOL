    ...
    Get-ChildItem -LiteralPath $root -Recurse -File -Include *.ts,*.tsx,*.js,*.mjs,*.md -ErrorAction SilentlyContinue | ForEach-Object {
      ...
      foreach($p in $Patterns){
        if($t.IndexOf($p,[System.StringComparison]::OrdinalIgnoreCase)-ge 0){
          $r=[System.IO.Path]::GetRelativePath($ProjectRoot,$_.FullName)
...
try {
  Write-Host "CASA School - Pre-Staging Real-World Flow Adjustment Preflight V1" -ForegroundColor Green
  ...
  $normalLate=HasAny $scanAll @("classifyCheckIn","\"LATE\"","'LATE'")
  ...
  $report.Add("CASA PRE-STAGING REAL-WORLD FLOW ADJUSTMENT PREFLIGHT V1")
  ...
  $last=$migrations | Select-Object -Last 1; $report.Add(""); $report.Add("[LATEST_MIGRATION]"); $report.Add("path=$([System.IO.Path]::GetRelativePath($ProjectRoot,$last.FullName))"); $report.Add("sha256=$(Sha $last.FullName)")
  ...
  Write-Host ""
  Write-Host "CASA PRE-STAGING REAL-WORLD FLOW ADJUSTMENT PREFLIGHT V1 IS COMPLETE" -ForegroundColor Green
