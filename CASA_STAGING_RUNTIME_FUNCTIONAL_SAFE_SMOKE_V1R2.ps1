...
Write-Host "CASA School - Staging Runtime + Functional Safe Smoke Verification V1R2" -ForegroundColor Green
Write-Host "Verifies the exact migration-30 source against Staging through a local Next.js runtime."
Write-Host "V1R2 corrects the canonical renewal-production.ts SHA pin to the exact V1R7 output; all runtime, schema, row-count and Production no-connect guards are unchanged."
...
try {
    ...
    $Evidence =
        Join-Path `
            $ProjectRoot `
            ".casa-backups\staging-runtime-functional-smoke-v1r2-$Stamp"
...
'@
    ...
    $Result =
        [ordered]@{
            createdAt =
                (Get-Date -Format o)
            proof =
                "CASA_STAGING_RUNTIME_FUNCTIONAL_SAFE_SMOKE_V1R2"
            classification =
                "GREEN"
            sourceAuthority =
                "main/a7aa668"
    ...
    Write-Host ""
    Write-Host "CASA STAGING RUNTIME + FUNCTIONAL SAFE SMOKE V1R2 IS GREEN" -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "CASA STAGING RUNTIME + FUNCTIONAL SAFE SMOKE V1R2 STOPPED: $($_.Exception.Message)" -ForegroundColor Red
