...
$AttemptId = "caa12c89-a355-41eb-b387-8eed946cf3b5"
...
Write-Host "CASA School - Latest-Pending Scanner Face/Liveness Live Recovery V1R1" -ForegroundColor Green
Write-Host "Development-only live biometric ceremony for the latest eligible already-card-matched Sep 9 pending attempt."
...
Write-Host "Live recovery target: $AttemptId"
Write-Host "Older duplicate preserved: $OlderAttemptId"
...
try {
    ...
    $EvidenceRoot =
        Join-Path `
            $ProjectRoot `
            ".casa-backups\scanner-latest-pending-face-live-recovery-v1r1-$stamp"
    ...
    if ((Sha $PendingRoutePath) -ne $ExpectedPendingRouteHash) {
        Stop-Phase "Duplicate-safe pending recovery route drift."
    }

    if ((Sha $RecoverySelftestPath) -ne $ExpectedRecoverySelftestHash) {
        Stop-Phase "Duplicate-safe pending recovery selftest drift."
    }
    ...
    Write-Host "  pending-attempt route:        DUPLICATE-SAFE / VERIFIED"
    Write-Host "  pending route SHA:            $ExpectedPendingRouteHash"
    Write-Host "  recovery selftest SHA:        $ExpectedRecoverySelftestHash"
...
let evidenceRows = [];
let presenceRows = [];
let latestEligiblePendingId = null;
let sameSessionAttendanceCount = 0;
let olderAttempt = null;
let olderLivenessCount = 0;
let olderProtectedCounts = null;
...
if (attempt) {
  ...
  latestEligiblePendingId =
    (
      await sql`
        select a.id
        from attendance_verification_attempts a
        where a.school_id=${attempt.school_id}::uuid
          and a.terminal_id=${attempt.terminal_id}::uuid
          and a.session_id=${attempt.session_id}::uuid
          and a.student_id=${attempt.student_id}::uuid
          and a.operation='CHECK_IN'
          and a.card_result='MATCHED'
      ...
      Number(
        (
          await sql`
            select count(*)::int as count
            from biometric_liveness_sessions
            where attempt_id=${olderAttemptId}::uuid
              and purpose='VERIFICATION'
          `
        )[0]?.count ??
        -1,
        ...
        await sql`
          select
            (
              select count(*)::int
              from student_attendance_records
              where source_attempt_id=${olderAttemptId}::uuid
            ) as attendance,
            (
              select count(*)::int
              from student_presence_events
...
'@
    ...
    if (
        @("ON_TIME","LATE") -notcontains
            [string]$Before.attempt.time_result -or
        $Before.attempt.scan_was_inside_window -ne $true -or
        [string]$Before.attempt.occurred_local_date -ne
            [string]$Before.attempt.attendance_date
    ) {
        Stop-Phase "Target attempt did not originate from an accepted scan-time CHECK_IN classification/window."
    }

    if (
        [string]$Before.latestEligiblePendingId -ne
        $AttemptId
    ) {
        Stop-Phase "Duplicate-safe pending route would not select the guarded latest attempt."
    }

    if ([int]$Before.sameSessionAttendanceCount -ne 0) {
        Stop-Phase "Attendance already exists for this student/session; pending recovery must not proceed."
    }

    if ($null -eq $Before.olderAttempt) {
        Stop-Phase "Older guarded duplicate attempt is missing."
    }

    if (
        [string]$Before.olderAttempt.id -ne $OlderAttemptId -or
        [string]$Before.olderAttempt.session_id -ne
            [string]$Before.attempt.session_id -or
        [string]$Before.olderAttempt.operation -ne "CHECK_IN" -or
        [string]$Before.olderAttempt.card_result -ne "MATCHED" -or
        @("ON_TIME","LATE") -notcontains
            [string]$Before.olderAttempt.time_result -or
        [string]$Before.olderAttempt.face_result -ne "NOT_RUN" -or
        [string]$Before.olderAttempt.liveness_result -ne "NOT_RUN" -or
        [string]$Before.olderAttempt.outcome -ne "PENDING" -or
        $null -ne $Before.olderAttempt.completed_at -or
        [int]$Before.olderLivenessCount -ne 0 -or
        [int]$Before.olderProtectedCounts.attendance -ne 0 -or
        [int]$Before.olderProtectedCounts.presence -ne 0 -or
        [int]$Before.olderProtectedCounts.evidence -ne 0
    ) {
        Stop-Phase "Older duplicate is no longer the untouched pending-history checkpoint."
    }
    ...
    Write-Host "  session:                      OPEN"
    Write-Host "  stored scan-time result:       $($Before.attempt.time_result) / ORIGINAL WINDOW VERIFIED"
    Write-Host "  current clock reclassification: NOT USED"
    Write-Host "  route-selected latest pending: $($Before.latestEligiblePendingId)"
    Write-Host "  older duplicate:              $OlderAttemptId / UNTOUCHED"
    ...
    try {
        ...
        if (
            ...
            [string]$BeforeBrowser.latestEligiblePendingId -ne $AttemptId -or
            [int]$BeforeBrowser.sameSessionAttendanceCount -ne 0 -or
            [string]$BeforeBrowser.olderAttempt.id -ne $OlderAttemptId -or
            [string]$BeforeBrowser.olderAttempt.outcome -ne "PENDING" -or
            [int]$BeforeBrowser.olderLivenessCount -ne 0 -or
        ...
        Write-Host "Latest recovery target: $AttemptId"
        Write-Host "Older duplicate that must remain untouched: $OlderAttemptId"
        Write-Host ""
        ...
        Write-Host "2. CASA should recover the LATEST eligible pending attempt automatically and show the student's identity with:"
        ...
        if (
            ...
            (Sha $PendingRoutePath) -ne $ExpectedPendingRouteHash -or
            (Sha $RecoverySelftestPath) -ne $ExpectedRecoverySelftestHash -or
        ...
        if (
            $null -eq $After.olderAttempt -or
            [string]$After.olderAttempt.id -ne $OlderAttemptId -or
            [string]$After.olderAttempt.operation -ne "CHECK_IN" -or
            [string]$After.olderAttempt.card_result -ne "MATCHED" -or
            [string]$After.olderAttempt.face_result -ne "NOT_RUN" -or
            [string]$After.olderAttempt.liveness_result -ne "NOT_RUN" -or
            [string]$After.olderAttempt.outcome -ne "PENDING" -or
            $null -ne $After.olderAttempt.completed_at -or
            [int]$After.olderLivenessCount -ne 0 -or
            [int]$After.olderProtectedCounts.attendance -ne 0 -or
            [int]$After.olderProtectedCounts.presence -ne 0 -or
            [int]$After.olderProtectedCounts.evidence -ne 0
        ) {
            Stop-Phase "Older duplicate changed during the live recovery ceremony."
        }
        ...
        if (-not $green) {
            ...
            $failed =
                [ordered]@{
                    ...
                    proof = "CASA_SCANNER_LATEST_PENDING_FACE_LIVE_RECOVERY_V1R1"
        ...
        if (
            [int]$After.sameSessionAttendanceCount -ne 1 -or
            $null -ne $After.latestEligiblePendingId
        ) {
            Stop-Phase "Duplicate-safe recovery suppression did not close after recorded attendance."
        }
        ...
        $result =
            [ordered]@{
                ...
                liveRecoveryTarget = $AttemptId
                olderDuplicatePreserved = $OlderAttemptId
                duplicateRecoverySuppressedAfterRecord = $true
                attemptPopulationUnchanged = $true
                cardRescanPerformed = $false
                card = "MATCHED"
                face = "PASSED"
        ...
        Write-Host ""
        Write-Host "CASA LATEST-PENDING SCANNER FACE/LIVENESS RECOVERY V1R1 IS GREEN" -ForegroundColor Green
        Write-Host "  live recovery target:          $AttemptId"
        ...
        Write-Host "  pending recovery after record: NONE / OLDER DUPLICATE SUPPRESSED"
        ...
        Write-Host ""
        Write-Host "NEXT: return this complete GREEN output. Development Scanner liveness/face acceptance is then closed and the older duplicate remains non-resumable after attendance records." -ForegroundColor Yellow
    }
    finally {
    ...
    Write-Host ""
    Write-Host "CASA LATEST-PENDING FACE/LIVENESS LIVE RECOVERY V1R1 STOPPED: $($_.Exception.Message)" -ForegroundColor Red
