...
Write-Host "CASA School - Vercel Staging Environment + OIDC Readiness V1" -ForegroundColor Green
...
try {
    ...
    $inspectResult =
        Invoke-Vercel `
            ...
            "Vercel linked project inspect"

    $inspectText = (@($inspectResult.stdout) -join "`n").Trim()

    if (
        $inspectText -notmatch [regex]::Escape($ExpectedProjectName) -and
        $inspectText -notmatch [regex]::Escape($ExpectedProjectId)
    ) {
        Fail "Vercel CLI did not resolve the linked casa-school project."
    }
    ...
    Step "Reading exact Vercel project configuration through authenticated API"
    ...
    $customEnvironmentProperty =
        $projectJson.PSObject.Properties[
            "customEnvironments"
        ]

    if (
        $null -ne $customEnvironmentProperty -and
        $null -ne $customEnvironmentProperty.Value
    ) {
        $customEnvironments =
            As-Array `
                $customEnvironmentProperty.Value
    }
    ...
    if ($stagingCustom.Count -eq 1) {
        ...
        elseif ($untargetedReady.Count -gt 0) {
            ...
            $classification =
                "NO_CUSTOM_STAGING__READY_UNTARGETED_DEPLOYMENT_PRESENT__DO_NOT_INFER_OIDC_ENVIRONMENT"
        }
    ...
    $evidence =
        Join-Path `
            $ProjectRoot `
            ".casa-backups\vercel-staging-environment-oidc-readiness-v1-$stamp"
    ...
    $result =
        [ordered]@{
            ...
            proof = "CASA_VERCEL_STAGING_ENVIRONMENT_OIDC_READINESS_V1"
            source = [ordered]@{
                branch = $branch
                head = $head
    ...
    Write-Host ""
    Write-Host "CASA VERCEL STAGING ENVIRONMENT + OIDC READINESS V1 IS GREEN" -ForegroundColor Green
    Write-Host "  source authority:               $branch/$head"
    Write-Host "  migrations:                     $migrationCount / VERIFIED"
    ...
    Write-Host ""
    Write-Host "CASA VERCEL STAGING ENVIRONMENT + OIDC READINESS V1 STOPPED: $($_.Exception.Message)" -ForegroundColor Red
